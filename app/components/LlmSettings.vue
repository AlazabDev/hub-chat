  ...
  <div class="h-full flex flex-col overflow-hidden">
    ...
    <div class="p-4 flex-1 space-y-6 overflow-y-auto">
      ...
      <div class="flex items-center justify-between">
        ...
        <USwitch v-model="llmParams.stream" :disabled="llmParams.useDeepseek" />
      </div>

      <div class="flex items-center justify-between">
        <div class="flex flex-col">
          <span>Use Deepseek</span>
          <span class="text-xs text-gray-500 dark:text-gray-400">Uses Deepseek API instead of Cloudflare Workers AI</span>
        </div>
        <USwitch v-model="llmParams.useDeepseek" @update:model-value="handleDeepseekToggle" />
      </div>
...
const _emit = defineEmits(["hideDrawer", "reset"]);

// Handle Deepseek toggle
function handleDeepseekToggle(useDeepseek: boolean) {
  if (useDeepseek) {
    // Disable streaming when using Deepseek as it's not supported
    llmParams.value.stream = false;
    // Set a default model that works with Deepseek
    llmParams.value.model = 'deepseek-chat';
  }
}
